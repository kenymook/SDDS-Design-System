...
Градиент редактируется как отдельный тип визуального значения в редакторе темы:

- пользователь выбирает тип градиента;
- управляет stop-значениями, цветом, прозрачностью и позицией;
- видит результат в preview сразу;
- изменение проходит тот же цикл Changes → validation → Publish;
- Viewer видит градиент только для просмотра.
